...
- **서버**: 고정된 **Ubuntu 22.04 워크스테이션** 1대. DeepAssist(8000) + LiteLLM(4000)이 이
  머신에서 돌고, vLLM은 이 서버에서(또는 서버가 도달 가능한 곳에서) `:8080`으로 서빙된다.
  `litellm/config.yaml`·`start.sh`·health의 `localhost`/`127.0.0.1`은 모두 **이 서버 호스트**
  기준이다 — 클라이언트 머신의 localhost가 아니다.
...
./start.sh                          # LiteLLM(4000)+DeepAssist(8000) 일괄 기동/정리 (vLLM은 외부, 시작 안 함)
...
| 바꾸는 것 | 함께 확인/수정할 곳 |
|---|---|
| **포트**(8000·4000·8080) | `.env(.sample)` · `litellm/config.yaml`(vLLM api_base 8080) · `start.sh` · `deepassist/config.py` · `README`·`CLAUDE` · **클라이언트 `gtas.serverUrl`(8000)** |
...
| **모델 라우팅** | `.env DEEPASSIST_MODEL` == `litellm/config.yaml`의 `model_name`. Agent SDK가 요청하는 모델명과 반드시 일치 |
| **vLLM 모델**(`litellm` `hosted_vllm/minimax-m2.5`) | 실제 vLLM 서빙 id(`VLLM_BASE_URL/v1/models`)와 일치해야 라우팅 성공 |
| **LiteLLM 포트/키**(`ANTHROPIC_BASE_URL`·`ANTHROPIC_AUTH_TOKEN`) | 주입 지점 **3곳**: `config.py`(env 기본값) · `main.py`(os.environ.setdefault) · `orchestrator._build_options`(options.env) |
