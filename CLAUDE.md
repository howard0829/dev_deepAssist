...
- **서버**: 고정된 **Ubuntu 22.04 워크스테이션 (Python 3.10+)** 1대. **DeepAssist(8000)만 이 리포가
  기동**하고, **LiteLLM(4000)·vLLM(8080)은 이 리포 밖에서 별도로 실행**한다(같은 서버든 다른 곳이든 무방).
...
- **클라이언트**: **Windows 또는 Linux** 중 하나의 VSCode (vsix 빌드는 Node.js 20 LTS+ 필요). 확장이
  파일/셸 도구를 **자기 OS에서 native 실행**한다. 서버는 클라이언트 워크스페이스를 직접 만지지
  않으며, 파일/셸은 위임 도구로 클라가 native 실행하고 서버는 경로를 **라벨로만 보유**한다.
...
- **기본: `main`에 직접 커밋·푸시.** 사용자가 "커밋해줘"/"푸시해줘"라고 할 때 **별도 브랜치 언급이
  없으면 새 브랜치를 만들지 말고 `main`에 바로 커밋하고 `origin/main`으로 푸시**한다. (기본 브랜치
  보호용 "브랜치 먼저 생성" 관습은 이 저장소에서 적용하지 않음 — 단일 사용자 로컬 운용)
- 브랜치/PR이 필요하면 **사용자가 명시적으로 요청할 때만** 생성한다.
- 커밋 메시지 컨벤션: 제목 `[FIX]/[DIAG]/docs` 등 + 본문 `증상/원인/수정/검증`, 마지막 줄
  `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`.
- 커밋 제외(.gitignore): `.env`(개인 설정)·`__pycache__`·`*.pyc`·`deepassist-vscode/node_modules`·
  `dist`·`*.vsix`. 템플릿 `.env.sample`은 추적한다.
