...
#   원격 공유 서버에서 vLLM(8080) + LiteLLM(4000) + DeepAssist(8000)를
#   한 번에 띄운다. Ctrl+C 시 세 프로세스를 모두 정리한다.
#
#   구동 토글(.env): START_VLLM / START_LITELLM
#   (이미 떠 있는 컴포넌트는 false로 두고 재사용)
# ──────────────────────────────────────────────────────────────
set -euo pipefail
...
VLLM_SERVED_NAME="${VLLM_SERVED_NAME:-deepassist-vllm}"
VLLM_GPU_MEMORY_UTILIZATION="${VLLM_GPU_MEMORY_UTILIZATION:-0.90}"
VLLM_MAX_MODEL_LEN="${VLLM_MAX_MODEL_LEN:-32768}"
export LITELLM_MASTER_KEY

VLLM_PID=""; LITELLM_PID=""; DEEPASSIST_PID=""
...
kill_port() {
  ...
  for pid in $pids; do
    ...
    case "$cmd" in
      python*|Python*|uvicorn|litellm|vllm*)
...
cleanup() {
  ...
  for pid in "$DEEPASSIST_PID" "$LITELLM_PID" "$VLLM_PID"; do
...
if [ "$START_VLLM" = "true" ]; then
  echo -e "\n${CYAN}🧠 vLLM 시작 (port $VLLM_PORT)${NC}"
  kill_port "$VLLM_PORT"
  if [ -z "$VLLM_MODEL" ]; then
    echo -e "${RED}   ❌ VLLM_MODEL 미설정 — .env에 모델 경로/ID 지정 필요${NC}"; exit 1
  fi
  ...
  VLLM_PID=$!
  wait_http "http://127.0.0.1:${VLLM_PORT}/v1/models" "vLLM" 120 || { cleanup; }
else
  echo -e "\n${YELLOW}⏭️  vLLM 시작 건너뜀 (START_VLLM=false)${NC}"
fi
